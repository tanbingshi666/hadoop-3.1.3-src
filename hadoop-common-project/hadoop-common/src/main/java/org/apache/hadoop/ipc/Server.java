...
public abstract class Server {
    ...
    private class RpcCall extends Call {
        ...
        void doResponse(Throwable t, RpcStatusProto status) throws IOException {
            ...
            // 将 Rpc Request 处理结果发送给 Client 端 (先 put 到当前 Connection 的 response 阻塞队列 等待 Responder 线程执行)
    ...
    private class Listener extends Thread {
                ...
                while (running) {
                    ...
                    try {
                        ...
                        for (int i = size; i > 0; i--) {
                            ...
                            // 往 Reader 线程的 NIO Selector 注册 OP_READ 事件并绑定 Connection
                        ...
                        while (iter.hasNext()) {
                            ...
                            try {
                                if (key.isReadable()) {
                                    // 监听到 OP_READ 事件 执行读取 SocketChannel 数据
                                    doRead(key);
                                }
        ...
        void doAccept(SelectionKey key) throws InterruptedException, IOException, OutOfMemoryError {
            ...
            while ((channel = server.accept()) != null) {
                ...
                // 封装 SocketChannel 为 Connection 对象并添加到 ConnectionManager 的 Set 集合
                ...
                // 绑定 Connection 到 SelectionKey 以便后续通过 SelectionKey.attachment() 获取对应的 Channel
                key.attach(c);  // so closeCurrentConnection can get the object
                // 将 Connection 添加到 Reader 线程的 LinkedBlockingQueue 阻塞队列并唤醒 Selector
    ...
    private class Responder extends Thread {
            ...
            try {
                synchronized (responseQueue) {
                    ...
                    if (!call.rpcResponse.hasRemaining()) {
                        ...
                        // 如果一次性返回不了全部数据给 Client 端 则将当前 RpcCall 重新塞入 response 阻塞队列 后续数据由 Responder 线程处理
                        ...
                        if (inHandler) {
                            ...
                            try {
                                ...
                                // 将当前 Rpc Request 的 SocketChannel 注册 OP_WRITE 事件到 Responder 线程的 NIO Selector
        ...
        void doRespond(RpcCall call) throws IOException {
            synchronized (call.connection.responseQueue) {
                ...
                // 将 Rpc Request 请求结果添加到当前 Connection 对象的 response 阻塞队列尾部
                call.connection.responseQueue.addLast(call);
                // 如果当前 Connection 对象的 response 阻塞队列大小为 1 直接有 Handler 线程返回 否则等待 Responder 线程处理
    ...
    public class Connection {
        ...
        private void processOneRpc(ByteBuffer bb)
            ...
            try {
                // 封装数据 ByteBuffer 为 Buffer 对象 (Buffer 底层还是维护一个 ByteBuffer)
                final RpcWritable.Buffer buffer = RpcWritable.Buffer.wrap(bb);
                // 读取数据区的 RpcRequestHeaderProto (通过依次读取字节 因为 RpcRequestHeaderProto 对象的字节固定)
        ...
        private void processRpcRequest(RpcRequestHeaderProto header,
            ...
            try {
                // 将 RpcCall 对象添加到 CallQueueManager 的 putRef 属性原子引用的阻塞队列中
        ...
        private void sendResponse(RpcCall call) throws IOException {
            // 将 Rpc Request 请求结果添加到当前 Connection 的 response 柱塞队列 等待 Responder 线程处理返回给 Client 端
    ...
    protected Server(String bindAddress, int port,
        ...
        if (queueSizePerHandler != -1) {
            ...
            // 每个 handler 线程队列最大接收多少个 call 默认值 100 (key = ipc.server.handler.queue.size)
            this.maxQueueSize = handlerCount * conf.getInt(
                    CommonConfigurationKeys.IPC_SERVER_HANDLER_QUEUE_SIZE_KEY,
                    CommonConfigurationKeys.IPC_SERVER_HANDLER_QUEUE_SIZE_DEFAULT);
        }
        // Server 端返回客户端请求最大数 默认值 1024*1024 (key = ipc.server.max.response.size)
        ...
        if (numReaders != -1) {
            ...
            // Server 端读取 socket 线程数 默认值 1 (key = ipc.server.read.threadpool.size)
            this.readThreads = conf.getInt(
                    CommonConfigurationKeys.IPC_SERVER_RPC_READ_THREADS_KEY,
                    CommonConfigurationKeys.IPC_SERVER_RPC_READ_THREADS_DEFAULT);
        }
        // Server 端连接 socket 最大阻塞数 默认值 100 (key = ipc.server.read.connection-queue.size)
    ...
    private class ConnectionManager {
        ...
        Connection register(SocketChannel channel, int ingressPort) {
            ...
            // 封装 SocketChannel 为 Connection 对象
            Connection connection = new Connection(channel, Time.now(), ingressPort);
            // 将 Connection 对象添加到 ConnectionManager 的 Set 集合
            add(connection);
            if (LOG.isDebugEnabled()) {
