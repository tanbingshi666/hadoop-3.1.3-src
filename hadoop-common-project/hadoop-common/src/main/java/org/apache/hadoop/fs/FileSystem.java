...
public abstract class FileSystem extends Configured
    ...
    public static URI getDefaultUri(Configuration conf) {
        URI uri = URI.create(
                // hdfs://hj101
                fixName(conf.get(FS_DEFAULT_NAME_KEY, DEFAULT_FS))
        );
    ...
    public FSDataInputStream open(Path f) throws IOException {
        ...
        // 请求 HDFS 下载文件返回 HdfsDataInputStream (最终调用 DistributedFileSystem.open())
            ...
            if (!FILE_SYSTEMS_LOADED) {
                ...
                while (it.hasNext()) {
                    ...
                    try {
                        fs = it.next();
                        try {
                            // hdfs -> DistributedFileSystem.class
