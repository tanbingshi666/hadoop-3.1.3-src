...
public class ActiveStandbyElector implements StatCallback, StringCallback {
    ...
    public synchronized void processResult(int rc, String path, Object ctx,
        ...
        // HA znode 节点既不是 ZkFC 创建 也不存在 可能出现 ZK 连接失败或者 ZK Session 超时需要重试
    ...
    private boolean becomeActive() {
        ...
        try {
            // fence 机制 判断 znode(/hadoop-ha/mycluster/ActiveBreadCrumb) 是否存在数据
    ...
    private void writeBreadCrumbNode(Stat oldBreadcrumbStat)
        ...
        if (oldBreadcrumbStat == null) {
            ...
            // 如果之前没有 NameNode active 则创建 znode(/hadoop-ha/mycluster/ActiveBreadCrumb)
    ...
    private Stat fenceOldActive() throws InterruptedException, KeeperException {
        ...
        if (Arrays.equals(data, appData)) {
            ...
            // NameNode 由 active 转为 standby (根据 data 数据区获取 NameNode 的 host + port
    ...
    private void createLockNodeAsync() {
        // 往 ZK 创建 znode 临时锁节点 (/hadoop-ha/mycluster/ActiveStandbyElectorLock)
        // 无论是否创建成功 将回调 ActiveStandbyElector.processResult(int,string,object,string)
    ...
    private final class WatcherWithClientRef implements Watcher {
        ...
        private void waitForZKConnectionEvent(int connectionTimeoutMs)
            ...
            try {
                // 等待连接 ZK 成功
                // 对于 zkfc 的 ActiveStandbyElector 连接 ZK 成功 则回调 WatcherWithClientRef.process()
