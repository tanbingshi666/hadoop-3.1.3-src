...
public class CapacityScheduler extends
    ...
    private void addApplication(ApplicationId applicationId, String queueName,
        ...
        try {
            ...
            // update the metrics
            queue.getMetrics().submitApp(user);
            // 创建 SchedulerApplication 并缓存
            SchedulerApplication<FiCaSchedulerApp> application =
                    new SchedulerApplication<FiCaSchedulerApp>(queue, user, priority);
            applications.put(applicationId, application);

            // Accepted application application_1684656010852_0005 from user: tanbs, in queue: default
            LOG.info("Accepted application " + applicationId + " from user: " + user
                    + ", in queue: " + queueName);

            // event = RMAppEvent
            // evenType = APP_ACCEPTED
            // 最终调用 ApplicationEventDispatcher.handler()
    ...
    private void addApplicationAttempt(
        ...
        try {
            ...
            // Added Application Attempt
            // appattempt_1684656010852_0005_000001 to scheduler from
            // user tanbs in queue default
            LOG.info("Added Application Attempt " + applicationAttemptId
                    + " to scheduler from user " + application.getUser() + " in queue "
                    + queue.getQueueName());

            // 默认 isAttemptRecovering = false
            if (isAttemptRecovering) {
                ...
                // 调用 ApplicationAttemptEventDispatcher.handle()
                rmContext.getDispatcher().getEventHandler().handle(
    ...
    public Allocation allocate(ApplicationAttemptId applicationAttemptId,
        ...
        try {
            ...
            // 创建 Allocation
            allocation = application.getAllocation(
                    getResourceCalculator(), //  YARN 集群资源计算器
                    getClusterResource(),    // YARN 集群资源管理
                    getMinimumResourceCapability() // 最少资源申请容量
            );
        } finally {
    ...
    private void allocateContainersToNode(NodeId nodeId,
        ...
        // 根据 NodeId 获取 NM 调度资源信息 FiCaSchedulerNode
        // 一个 NodeId 对应一个 FiCaSchedulerNode
        FiCaSchedulerNode node = getNode(nodeId);
        if (null != node) {
            int offswitchCount = 0;
            int assignedContainers = 0;

            // 候选 NM (也即该 NM 可能有资源分配给其他 Application)
            CandidateNodeSet<FiCaSchedulerNode> candidates =
                    new SimpleCandidateNodeSet<>(node);
            // 分配容器(这些容器需要分配资源)给 NM
    ...
    private CSAssignment allocateOrReserveNewContainers(
            CandidateNodeSet<FiCaSchedulerNode> candidates,
            boolean withNodeHeartbeat) {
        // 选择队列分配资源给容器 (底层将会使用深度优先遍历算符找到对应的叶子队列)
        // 先调用 ParentQueue 然后调用 LeafQueue
    ...
    protected void completedContainerInternal(
        ...
        FiCaSchedulerApp application = getCurrentAttemptForContainer(
                container.getId());
        // 根据容器 ApplicationAttemptId 获取 ApplicationId
        ApplicationId appId =
        ...
        // 根据容器 ID (也即 NM 地址) 获取 NM 的 FiCaSchedulerNode 相关信息
        ...
        // Inform the queue
        // 获取已经完成容器的 Application 对应的队列 (叶子队列)
        LeafQueue queue = (LeafQueue) application.getQueue();
        // 相应队列进行资源回收 (回收已经完成容器的资源)
