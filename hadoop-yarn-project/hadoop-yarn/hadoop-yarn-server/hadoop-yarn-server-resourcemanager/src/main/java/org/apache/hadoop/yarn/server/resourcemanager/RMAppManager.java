...
public class RMAppManager implements EventHandler<RMAppManagerEvent>,
    ...
    protected void submitApplication(
        ...
        try {
            if (UserGroupInformation.isSecurityEnabled()) {
                ...
                // gets started.
                // ResourceManager 的异步事件分发器提交 RMAppEventType.START 事件
                // 最后调用 ApplicationEventDispatcher.handler()
    ...
    private RMAppImpl createAndPopulateNewRMApp(
        ...
        ApplicationId applicationId = submissionContext.getApplicationId();
        // 校验启动 AM 资源 (AM 默认容器资源请求 内存 1536 MB 1 CPU *)
        ...
        if (!isRecovery &&
                // 默认不启动 ACL
                YarnConfiguration.isAclEnabled(conf)
        ...
        RMAppImpl application =
                new RMAppImpl(
                        applicationId, // 提交任务 ID
                        rmContext,     // ResourceManager 的上下文对象
                        this.conf,     // YARN 集群配置信息
                        submissionContext.getApplicationName(), // 提交任务名称
                        user, // 提交任务的用户
                        submissionContext.getQueue(), // 提交任务队列
                        submissionContext, // 提交任务上下文对象
                        this.scheduler,    // 资源调度器 默认 CapacityScheduler
                        this.masterService, // 管理 AM 服务 默认 ApplicationMasterService
                        submitTime,         // 任务提交时间 默认系统时间
                        submissionContext.getApplicationType(), // 任务提交类型 比如 MAPREDUCE
                        submissionContext.getApplicationTags(), // 任务提交标签
                        amReqs,          // 启动 AM 资源请求信息
                        placementContext,  //
                        startTime        // 启动任务时间 默认 -1
                );
        // Concurrent app submissions with same applicationId will fail here
        // Concurrent app submissions with different applicationIds will not
        // influence each other
        // 缓存 AM 信息 (ApplicationId -> RMApp)
    ...
    private List<ResourceRequest> validateAndCreateResourceRequest(
        ...
        if (!submissionContext.getUnmanagedAM()) {
            ...
            try {
                ...
                // 默认启动 AM 只有一个资源请求 (默认内存 1536 MB 1 CPU *)
                for (ResourceRequest amReq : amReqs) {
                    // 重新赋值启动 AM 资源信息
                ...
                for (ResourceRequest amReq : amReqs) {
                    ...
                    // 判断启动 AM 资源是否合法 (默认情况下都是启动 AM 时申请的资源信息)
                    amReq.setCapability(
