...
public class JournalNode implements Tool, Configurable, JournalNodeMXBean {
    ...
    static {
        // 调用 HdfsConfiguration 静态代码块 添加 hdfs-default.xml hdfs-site.xml 文件名到 defaultResources 容器中
    ...
    private File getLogDir(String jid, String nameServiceId) throws IOException {
        ...
        if (dir == null) {
            // 一般在 hdfs-site.xml 配置 key = dfs.journalnode.edits.dir
            // 比如配置 value = /opt/app/hadoop-3.1.3/data/jn
            dir = conf.get(DFSConfigKeys.DFS_JOURNALNODE_EDITS_DIR_KEY,
                    DFSConfigKeys.DFS_JOURNALNODE_EDITS_DIR_DEFAULT);
        }
        ...
        assert jid != null;
        // 创建子文件 比如 /opt/app/hadoop-3.1.3/data/jn/mycluster
