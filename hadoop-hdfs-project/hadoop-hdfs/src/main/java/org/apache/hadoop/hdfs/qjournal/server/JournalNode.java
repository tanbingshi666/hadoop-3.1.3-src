...
public class JournalNode implements Tool, Configurable, JournalNodeMXBean {
    ...
    static {
        // 通过静态的方法加载 (并未解析) classpath 的 core-default.xml core-site.xml hdfs-default.xml hdfs-site.ml
    ...
    public void setConf(Configuration conf) {
        ...
        // 里面需要初始化加载四大文件 (classpath 路径下的 core-default.xml core-site.xml hdfs-default.xml hdfs-site.xml)
        // 默认为 empty
        ...
        if (nameserviceIds.size() < 2) {
            ...
            // 获取 JournalNode editLog Dir
            // 默认为 /tmp/hadoop/dfs/journalnode/
    ...
    public void start() throws IOException {
        ...
        try {
            ...
            // localDir 默认只有一个路径 /tmp/hadoop/dfs/journalnode/
            for (File journalDir : localDir) {
                // 检查 journal 是否合法 是否可以创建
            ...
            // 创建 JournalNodeHttpServer
            httpServer = new JournalNodeHttpServer(conf, this,
                    // 默认 	0.0.0.0:8480
                    getHttpServerBindAddress(conf));
            // 启动 JournalNodeHttpServer
            httpServer.start();

            httpServerURI = httpServer.getServerURI().toString();

            // 创建 JournalNodeRpcServer
            rpcServer = new JournalNodeRpcServer(conf, this);
            // 启动 JournalNodeRpcServer
            rpcServer.start();
