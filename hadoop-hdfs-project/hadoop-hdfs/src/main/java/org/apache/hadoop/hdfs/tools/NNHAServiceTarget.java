...
public class NNHAServiceTarget extends HAServiceTarget {
    ...
    public NNHAServiceTarget(Configuration conf,
        ...
        // 优先获取 key = dfs.namenode.servicerpc-address.${nameservice}.${namenode.id}
        // 其次获取 key = dfs.namenode.rpc-address.${nameservice}.${namenode.id}
        String serviceAddr =
        ...
        // 获取 key = dfs.ha.automatic-failover.enabled 对应的值 一般在 hdfs-site.xml 配置为 true
        ...
        // 获取 key = dfs.ha.zkfc.port 对应的值 默认为 8019
        ...
        try {
            // 获取 key = dfs.ha.fencing.methods 对应的值, 解析该值创建 NodeFencer
