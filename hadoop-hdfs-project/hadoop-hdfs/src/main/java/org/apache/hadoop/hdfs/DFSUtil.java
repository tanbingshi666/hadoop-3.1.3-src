...
public class DFSUtil {
    ...
    private static String getNameServiceId(Configuration conf, String addressKey) {
        // 获取 key = dfs.nameservice.id 对应的值 value (一般 NameNode HA 配置 nameservice 在 hdfs-site.xml)
        String nameserviceId = conf.get(DFS_NAMESERVICE_ID);
        if (nameserviceId != null) {
            return nameserviceId;
        }

        // 如果没有在 hdfs-site.xml 没有配置 dfs.nameservice.id 对象的值 那么获取 key = dfs.nameservice 对象的值
