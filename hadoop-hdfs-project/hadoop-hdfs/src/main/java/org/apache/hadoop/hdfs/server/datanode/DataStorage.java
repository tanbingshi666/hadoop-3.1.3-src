...
public class DataStorage extends Storage {
    ...
    static int getParallelVolumeLoadThreadsNum(int dataDirs, Configuration conf) {
        ...
        // Using 1 threads to upgrade data directories (dfs.datanode.parallel.volumes.load.threads.num=1, dataDirs=1)
    ...
    synchronized List<StorageDirectory> addStorageLocations(DataNode datanode,
        ...
        // 创建线程池 大小为 DataNode 数据存储目录大小 比如 1
        final ExecutorService executor = Executors.newFixedThreadPool(numThreads);
        try {
            // 从磁盘加载 DataNode 数据存储信息
